# What has changed: 
# multihead attetion splits one massive channel (n_embed) 
# of communication between tokens in separate channels, 
# so that they can learn about different features independently
# 
# eval loss down to 2.19
...
logging.basicConfig(level=logging.DEBUG, filename=f"out_04.log")
...
class TransformersLanguageModel(nn.Module):
    def __init__(self):
        ...
        self.sa_heads = MultiHeadAttention(num_heads=4, head_size=n_embd // 4) # we want to eventually keep the same n_embed size, but meanwhile we split it into chunks, process and concatenate
